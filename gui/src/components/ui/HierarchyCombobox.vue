...
const props = defineProps({
  ...
  // Off by default — DataPoints/KNX-device linking deliberately needs every
  // tree, including ones auto-generated by an ETS import. The Logic
  // hierarchy-assignment picker is the first consumer that must NOT offer
  // those: assigning a logic sheet into an imported KNX-location tree makes
  // no organisational sense there, so it opts in to hide any tree whose
  // `source` is non-empty (currently only "ets_import:<mode>").
  manualTreesOnly: { type: Boolean, default: false },
})
...
async function load() {
  try {
    const { data: allTrees } = await hierarchyApi.listTrees()
    if (!Array.isArray(allTrees)) return
    const trees = props.manualTreesOnly ? allTrees.filter((t) => !t.source) : allTrees
