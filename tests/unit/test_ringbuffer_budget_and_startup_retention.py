...
@pytest.mark.asyncio
async def test_startup_retention_defers_while_migration_in_progress(tmp_path: Path):
    """Der Startup-``enforce_retention`` löscht die attached Legacy NICHT, solange ein ``migrating``-Segment existiert (#951, F2).

    Reproduziert den Startup-Datenverlust: über-Budget attached Legacy + versteckter
    ``migrating``-Chunk + eine positive Live-v2-Zeile (No-Zero-History-Guard erfüllt) beim
    (Neu-)Start. Ohne Deferral löschte der unbedingte Startup-Retention-Pass die noch attached,
    nicht fertig kopierte Legacy-Quelle.
    """
    disk_path = tmp_path / "obs_ringbuffer.db"
    await _seed_legacy(disk_path)

    # Erstinstanz nur, um den migrierenden Zustand auf der Platte zu erzeugen:
    # ein ``migrating``-Segment + eine positive Live-Zeile, dann sauber stoppen.
    prep = RingBuffer(storage="file", disk_path=str(disk_path), segmented=True, segment_max_rows=1, max_entries=None)
    await prep.start()
    try:
        assert len(await prep.store.manifest.list_legacy_segments()) == 1
        await _mark_a_closed_segment_migrating(prep)
        # Positive aktive Zeile erhalten (erfüllt beim Restart den No-Zero-History-Guard),
        # ohne die migrating-Segmente zu promoten.
        prep._segment_max_rows = 1000
        await _record(prep, 200, "2026-06-01T00:00:00.000Z")
        assert len(await prep.store.manifest.list_migrating_segments()) == 1
    finally:
        await prep.stop()

    # Neustart WÄHREND der Migration: hartes Size-Budget von 1 Byte über den Konstruktor,
    # damit die attached über-Budget-Legacy beim Startup-Retention-Pass reclaimbar WÄRE.
    rb = RingBuffer(
        storage="file",
    ...
    await rb.start()
    try:
        # Deferral greift: die attached Legacy-Quelle steht noch (kein Datenverlust),
        # solange die Migration (migrating-Segment) läuft.
        assert len(await rb.store.manifest.list_migrating_segments()) == 1
...
@pytest.mark.asyncio
async def test_startup_retention_runs_without_migrating_segments(tmp_path: Path):
    """Gegentest: ohne ``migrating``-Segmente läuft die Startup-Retention normal (#951, F2).

    Reiner Legacy-Bestand, KEINE laufende Migration, hartes 1-Byte-Budget: der Startup-Pass
    darf – sobald ein frisches v2-Segment eine positive Zeile trägt – die über-Budget-Legacy
    zurückgewinnen. Hier wird verifiziert, dass ``_migration_in_progress()`` beim Normalstart
    False ist und der Startup-``enforce_retention`` nicht ausgesetzt wird.
    """
    disk_path = tmp_path / "obs_ringbuffer.db"
    await _seed_legacy(disk_path)
    ...
    await rb.start()
    try:
        # Kein migrating-Segment → Deferral greift NICHT.
        assert await rb._migration_in_progress() is False
        # Beim reinen Legacy-Bestand ohne positive v2-Zeile schützt der No-Zero-History-Guard
        # die attached Legacy noch. Sobald ein frischer v2-Append eine positive Zeile erzeugt,
        # greift die (nicht deferrte) append-getriebene Retention und gibt die Legacy frei.
