...
### Fixes 🐞
* Backend: Adapter binding loads now use a composite OBS.db index on `adapter_bindings(adapter_instance_id, enabled)`, avoiding full scans during adapter start and reload without adding broad audit-only index candidates. https://github.com/abeggled/openbridgeserver/issues/935
