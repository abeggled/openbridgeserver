...
<template>
  <nav v-if="breadcrumb.length > 0" class="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 flex-wrap">
    <button @click="router.push({ name: 'tree' })" class="hover:text-gray-900 dark:hover:text-gray-200 transition-colors">
      ⌂
    </button>
    <template v-for="(node, idx) in breadcrumb" :key="node.id">
      ...
      <button
        ...
        :class="idx === breadcrumb.length - 1 ? 'text-gray-800 dark:text-gray-200 font-medium pointer-events-none' : 'hover:text-gray-900 dark:hover:text-gray-200'"
