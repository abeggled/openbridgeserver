...
| **Edge detection** | In, Reset | Out, Rising, Falling | Evaluates the input as a boolean and reacts to the transition: a rising edge (false → true) outputs the configured rising value and sets the Rising trigger, a falling edge (true → false) the falling value and the Falling trigger. Without an edge nothing is sent on Out, so a downstream Write Object does not write on every run. Sending can be disabled per edge, the block can be limited to one edge direction, and Reset drops the remembered level so the next value starts fresh. |
